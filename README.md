...
Custom Exceptions — `OutOfStockException` and `InvalidPaymentException` provide descriptive error messages when rules are violated. An additional business-rule exception blocks inventory changes while a purchase is in progress, preventing stock inconsistencies.
Project Structure
```
src/
